...
export const getPatient = async (req: Request, res: Response): Promise<Response<Patient>> => {
    ...
    try{
        ...
        if((result[0] as Array<ResultSet>).length > 0) {
            ...
            return res.status(Code.NOT_FOUND).send(new HttpResponse(Code.NOT_FOUND, Status.NOT_FOUND, 'Patients not found'));
        }
...
export const createPatient = async (req: Request, res: Response): Promise<Response<Patient>> => {
    ...
    try{
        ...
        patient = { id: (result[0] as ResultSetHeader).insertId, ...req.body};
            return res.status(Code.OK).send(new HttpResponse(Code.OK, Status.OK, 'Patient created', patient));
    } catch(error: unknown){
        console.error(error);
        return res.status(Code.INTERNAL_SERVER_ERROR).send(new HttpResponse(Code.INTERNAL_SERVER_ERROR, Status.INTERNAL_SERVER_ERROR, 'An error occured'));
    }
